...
export async function runOccupation(
  ...
  const ctxRaw = await retrieveForTask(sql, { userId: opts.userId, roleId: role.id });
  // Synthetic verification records never feed real work.
  const ctx = ctxRaw.filter((c) => task.is_test_only === 1 || !c.body.startsWith("TEST_ONLY"));
