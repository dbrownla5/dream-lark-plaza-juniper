...
async function createPgliteSql(): Promise<Sql> {
  // File-backed Postgres (DATA_DIR/pglite). Survives process restart. Neon
  // replaces this when DATABASE_URL is set.
  globalRef.__pgliteInstance__ ??= (async () => {
    const { PGlite } = await import("@electric-sql/pglite");
    const { ensureDataDirs, pgliteDir } = await import("./os/data-dir.ts");
    ensureDataDirs();
